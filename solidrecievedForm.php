...
<td><input class="form-control" id="tanker" type="text" name="tanker_no" placeholder="enter the tanker number" align="center" style="width:380px;font-size:10pt" ng-model="tanker_no"></td>
</tr>
...
if(isset($_POST['save']))
{
	$tanker="select * from ftd_solid_recvd_plant where Tran_Date='".$_POST['date']."' and Tanker_No='".$_POST['tanker_no']."' ";
	$result=mysqli_query($db,$tanker);
	if(mysqli_num_rows($result)>0)
	{
		echo '<script>alert("Tanker no '.$_POST['tanker_no'].' data already available")</script>';
	}
	else if(mysqli_num_rows($result)==0)
	{
	$bmc=explode("-",$_POST['bmc']);
	$insert="insert into ftd_solid_recvd_plant(Tran_Date,Fran_Name,Tanker_No,Bmc_Code,Bmc_Name,Qty,Fat,Snf,Add_Date,Add_user,D_Flag) values('".$_POST['date']."','".$_POST['Franchise']."','".$_POST['tanker_no']."','".$bmc[0]."','".$bmc[1]."',".$_POST['quantity'].",".$_POST['fat'].",".$_POST['snf'].",'".date("Y-m-d")."','".$_SESSION['login_user']."','A');";
...
$check="select * from ftd_solid_recvd_plant where Tran_Date='".date("Y-m-d")."'";
	...
	//echo '<p style="color:blue;">'.mysqli_num_rows($result)." ".date("Y-m-d").'</p>';
	if(mysqli_num_rows($result)>0)
	{
